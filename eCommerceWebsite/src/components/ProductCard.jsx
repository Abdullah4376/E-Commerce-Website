...
function ProductCard({$id, title, featuredImage, category, price, brand}) {
    return(
        <>
                <div className="p-3 rounded-md bg-white group shadow-2xl mb-2">
                        <div className="overflow-hidden relative">
                            
                            <img src={service.getFilePreview(featuredImage)} alt="" className="rounded-t-md group-hover:scale-110 group-hover:brightness-50 duration-500"/>
                            ...
                            <div className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] opacity-0 duration-500 group-hover:opacity-100 text-center">
                                <Link to={`/${brand}/product-${$id}`} className="px-10 duration-500 rounded-md py-2 font-normal bg-white mb-3 hover:text-white hover:bg-[#5F3AFC]">Buy Now</Link>
